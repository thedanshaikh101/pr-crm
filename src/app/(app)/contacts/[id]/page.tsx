        ...
        <div className="space-y-4">
          <header className="card flex items-start gap-4 p-5">
            ...
            <div className="min-w-0 flex-1">
              ...
              {c.email && <div className="mt-1 flex flex-wrap items-center gap-1 text-sm">{c.email} {c.emailStatus === "VALID" && <span className="pill bg-green-50 text-good">verified</span>}{c.emailStatus === "RISKY" && <span className="pill bg-amber-50 text-warn">risky</span>}{(c.emailStatus === "BOUNCED" || c.emailStatus === "INVALID") && <span className="pill bg-red-50 text-bad">{c.emailStatus.toLowerCase()}</span>}
                {!["BOUNCED", "COMPLAINED", "UNSUBSCRIBED"].includes(c.emailStatus) && <form action={async () => { "use server"; await verifyContactNow(c.id); }}><button className="btn px-2 py-0.5 text-xs" title="Syntax and mail-server check">{c.emailVerifiedAt ? `Re-verify (checked ${c.emailVerifiedAt.toLocaleDateString()})` : "Verify email"}</button></form>}</div>}
